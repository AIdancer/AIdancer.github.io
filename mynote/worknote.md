...
## OpenAFS状态监控
&emsp;&emsp;OpenAFS是一个分布式文件系统。OpenAFS,有着卡耐基梅隆大学的支持,OpenAFS提供有架构,文件共享客户端,服务器,具有只读形式的共享,单个客户端位置的独立性,可扩展,安全,对用户透明共享资源的能力.是一个成熟稳定的文件系统。  
&emsp;&emsp;摩根士丹利目前采用该文件系统作为公司内部的公共存储空间，刚开始很瞧不起这个文件系统，毕竟当红炸子鸡是HDFS，后来发现OpenAFS还是有很多优点的，虽然没有HDFS的社区那么好，但是使用过程贴近windows系统，像摩根这种windows资深用户，AFS盘可以看起来像普通磁盘一样挂载到系统里面，用起来着实方便。  
项目涉及点：  
...
## 摩根士丹利自动化运维
&emsp;&emsp;嗯，目前来说，这是我在Morgan做的最喜欢的项目了。  
&emsp;&emsp;摩根有很多组，每个组有很多的服务，这些服务都需要监控，比如外汇交易部的人把网线踢掉了，你要系统自动打电话给他让他赶紧去修-，-  
做完大概有这么几个点：
  - 前端angular系列
...
## 幻方Alpha回测系统
&emsp;&emsp;说好的当CTA基金经理呢，怎么一直被安排在quant dev岗位上，而且梁文锋居然不搞CTA了，擦，衰啊。  
&emsp;&emsp;全司C++系，还有Python，是我喜欢的技术栈。
目前项目点：
...
## Spark相关
&emsp;&emsp;以前总觉得Spark很厉害，直到我自己部署了Spark，且写了各种业务代码。目前为止，Spark最大的用途就是大屏，其次可以加速许多算法的统计速度。  
主要管理点：  
  - 第一次管人，带三个小朋友做大数据
  - 做了好多次面试官，谦逊很重要啊
  - 自己技术过硬，才能让小朋友尊重
...
## Flink相关
&emsp;&emsp;同上，Flink确实快，流式处理名不虚传，管理界面也比Spark舒服一点。
...
## 基于ES的问答系统
&emsp;&emsp;FAQ-Bot，智能问答  
  - 分级标题
  - 文档存储（ES）
  - 如果用户点击各类分集标题索引的话，就给他们递归展开
  - 不点乱输入的话直接ES match，就是这么简单，发现有很多网站的问答系统甚至搜索都是这么做的哈哈哈。

&emsp;&emsp;（update 2023-01-31，不是所有的智能问答都是chatGPT）  

## 济南实时公交到站时间预测
&emsp;&emsp;居然还申请了专利，我一个字没写就是一作，原创buff哈哈哈。  
主要关键点记录下：  
...
## 文财私募基金回测平台
&emsp;&emsp;没太多想说的，基本复现了幻方的回测平台，只是数据上有好多欠缺。  
&emsp;&emsp;(2019-12-06 update)老板不懂量化真的很蛋疼。  
&emsp;&emsp;(2020-2-20 update)招人了，预估要带5人团队。  
主要技术点：
  - C++/Python
  - ES做股票tag数据库，方便做概念检索
...
## 国标资产股票交易系统
&emsp;&emsp;中泰XTP是真的好用啊，C++系列Nice。  

## 量化策略开发
&emsp;&emsp;本部分涉及核心机密，就不公开了，总体上来说：
  - **Linear模型用好其实已经很香了**
  - LGBM和XGBoost都很好用
...
## 监控平台开发
&emsp;&emsp;浪潮信息的IncloudOS，私有云平台（基于Kubernetes和OpenStack），其监控系统目前涉及技术如下：
